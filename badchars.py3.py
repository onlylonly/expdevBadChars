...
class BytesParser():
    ...
    def __init__(self, input, name = None, format = None):
        ...
        if format:
            ...
            if str(format).lower() == "raw":
                ...
                assert (format in list(BytesParser.formats_rex.keys()) or self.format is not None), \
    ...
    def normalize_input(self):
        input = []
        for line in self.input.decode('latin-1').split('\n'):
            line = line.strip()
    ...
    def interpret_format_name(name):
        for k, v in list(BytesParser.formats_aliases.items()):
            if name.lower() in v:
                return k
    ...
    def compile_regexps():
        if len(BytesParser.formats_compiled) == 0:
            for name, rex in list(BytesParser.formats_rex.items()):
    ...
    def recognize_format(self):
        for line in self.input.decode('latin-1').split('\n'):
            ...
            for format, rex in list(BytesParser.formats_compiled.items()):
    ...
    def fetch_bytes(self):
        ...
        if self.format == 'raw':
            ...
            #in python3.x, self.input is already in type: bytes, ord() is not needed anymore 
            self.bytes = self.input
            return len(self.bytes) > 0
        
        for line in self.input.decode('latin-1').split('\n'):
            callback_called = False
            if self.format in list(BytesParser.formats_callbacks.keys()) and \
...
def bad_chars(comp):
    mapped_chunks = list(map(''.join, comp.guess_mapping()))
    buffer1 = [chr(c) for c in buffers[0]]
    mapping = list(zip(buffer1, mapped_chunks))
...
def guess_bad_chars(comp):
    ...
    bytes_omitted_from_input = set(map(chr, list(range(0, 256)))) - set(comp.x)
...
class MemoryComparator(object):
    ...
    def get_blocks(self):
        ...
        for i, j in zip(list(range(i, len(x))), itertools.count(j)):
    ...
    def guess_mapping(self):
        ...
        for b, values in mappings_by_byte.items():
            mappings_by_byte[b] = sorted(list(values.items()),
                                     key=lambda value_count: (-value_count[1], -len(value_count[0])))
...
class HexDumpPrinter:
    ...
    def construct_comparator_dump(self, mapping):
        ...
        for i, chunk in enumerate(HexDumpPrinter.extract_chunks(mapping)):
            ...
            line1 += '| ' + ''.join([x if ord(x) >= 0x20 and ord(x) < 0x7f else '.' for x in src]).ljust(16, ' ')
            ascii2 = '| '
